...
from app.database.mongo import MONGO_DB_NSE
...
nse_stock_list = StocksList()
stock_symbols = nse_stock_list.get_stock_list()
STOCK_NAME_AND_SYMBOL_DICT = {}

for stock_name, stock_symbol in stock_symbols:
    STOCK_NAME_AND_SYMBOL_DICT[stock_symbol] = stock_name

bucket_gains_collection = MONGO_DB_NSE["bucket-gains"]


def add_nse_bucket(current_user: UsersTable, input_bucket: BucketSchema):
    """
    add nse bucket to bucket_gains_tracker_mongodb database collection bucket-gains.
    """
...
def get_all_nse_buckets(current_user: UsersTable):
    """
...
def get_nse_bucket(current_user: UsersTable, object_id: str):
    """
...
def soft_delete_nse_bucket(current_user: UsersTable, object_id: str):
    """
...
def update_nse_bucket(
    ...
    for stock_code in updated_stock_codes:
        ...
        if updated_bucket_buying_date < earliest_date_data_available:
            raise ValueError(
                f""" update...bucket_buying_date is older than earliest_date_data_available for stock {stock_code}! Try Decreasing the bucket period."""
            )
...
def get_nse_bucket_profit(current_user: UsersTable, id: str):
    """
    ...
    for i in range(len(bucket["bucket_stocks"])):
        ...
        stock = StockNse(bucket["bucket_stocks"][i]["stock_code"])
